...
class ProteinLigand3DDataset(Dataset):
    def __init__(self, dataframe, pdb_root, device='cpu', dist_cutoff=8.0, max_prot_len=1000):
        self.data = dataframe
    ...
    def get_atom_features(self, atom):
        ...
        hybrid_list = [Chem.rdchem.HybridizationType.SP, Chem.rdchem.HybridizationType.SP2, Chem.rdchem.HybridizationType.SP3, Chem.rdchem.HybridizationType.SP3D, Chem.rdchem.HybridizationType.SP3D2]
    ...
    def parse_pdb_structure(self, pdb_id):
        ...
        sequence = ""
        res_codes = {
            'ALA':'A', 'ARG':'R', 'ASN':'N', 'ASP':'D', 'CYS':'C', 'GLN':'Q', 'GLE':'E', 'GLY':'G', 'HIS':'H',
        ...
        if not ca_coords: return None, None
        return np.array(ca_coords), sequence

    @torch.no_grad()
    def get_esm_embeddings(self, sequence):
        sequence = sequence[:self.max_prot_len]
        batch_labels, batch_strs, batch_tokens = self.esm_batch_converter([("protein", sequence)])
    ...
    def __getitem__(self, idx):
        ...
        ligand_x = torch.tensor(np.array([self.get_atom_features(a) for a in mol.GetAtoms()]), dtype=torch.float)
        ligand_edges = []
        for b in mol.GetBonds():
            ligand_edges.extend([(b.GetBeginAtomIdx(), b.GetEndAtomIdx()), (b.GetEndAtomIdx(), b.GetBeginAtomIdx())])
