...
def train():
    ...
    dataset = SOTABindingDataset(df, pdb_path, device=device, esm_model_name="esm2_t33_650M_UR50D")
    
    train_size = int(0.8 * len(dataset))
    val_size = len(dataset) - train_size
    train_dataset, val_dataset = random_split(dataset, [train_size, val_size])
    
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn_sota)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_sota)
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}: Train NLL: {avg_train_loss:.4f}, Val MSE: {avg_val_loss:.4f}")
        
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            torch.save(model.state_dict(), "research/gnn_gnn_experiment/best_model_sota.pth")
