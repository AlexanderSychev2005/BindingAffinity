...
class GNNGNNModel(nn.Module):
    def __init__(self, ligand_in_dim, protein_in_dim, hidden_dim=256, n_heads=4, dropout=0.2):
        super().__init__()
        ...
        # Protein Encoder
        self.p_conv1 = GATv2Conv(protein_in_dim, hidden_dim, heads=n_heads, concat=False)
...
if __name__ == "__main__":
    ...
    # Fake batch
    l1 = Data(x=torch.randn(10, l_in), edge_index=torch.tensor([[0,1],[1,0]], dtype=torch.long))
    l2 = Data(x=torch.randn(15, l_in), edge_index=torch.tensor([[0,1],[1,0]], dtype=torch.long))
    l_batch = Batch.from_data_list([l1, l2])
    
    p1 = Data(x=torch.randn(100, p_in), edge_index=torch.tensor([[0,1],[1,0]], dtype=torch.long))
    p2 = Data(x=torch.randn(120, p_in), edge_index=torch.tensor([[0,1],[1,0]], dtype=torch.long))
