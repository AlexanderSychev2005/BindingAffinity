...
def get_atom_features(atom):
    ...
    chirality_list = [Chem.rdchem.ChiralType.CHI_UNSPECIFIED, Chem.rdchem.ChiralType.CHI_TETRAHEDRAL_CW, Chem.rdchem.ChiralType.CHI_TETRAHEDRAL_CCW, Chem.rdchem.ChiralType.CHI_OTHER]
    ...
    return np.array(
        ...
        one_of_k_encoding(atom.GetHybridization(), [Chem.rdchem.HybridizationType.SP, Chem.rdchem.HybridizationType.SP2, Chem.rdchem.HybridizationType.SP3, Chem.rdchem.HybridizationType.SP3D, Chem.rdchem.HybridizationType.SP3D2, "other"]) +
        [atom.GetIsAromatic()] +
...
def get_residue_features(char):
    ...
    """
    res_list = ['A', 'R', 'N', 'D', 'C', 'Q', 'E', 'G', 'H', 'I', 'L', 'K', 'M', 'F', 'P', 'S', 'T', 'W', 'Y', 'V', 'X']
...
class ProteinLigandGraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ligand_edge_index = torch.tensor(ligand_edges, dtype=torch.long).t().contiguous()
...
def collate_fn(batch):
    ...
    for item in batch:
        ligand_list.append(Data(x=item['ligand_x'], edge_index=item['ligand_edge_index']))
        protein_list.append(Data(x=item['protein_x'], edge_index=item['protein_edge_index']))
        y_list.append(item['y'])
        
    return Batch.from_data_list(ligand_list), Batch.from_data_list(protein_list), torch.stack(y_list)
