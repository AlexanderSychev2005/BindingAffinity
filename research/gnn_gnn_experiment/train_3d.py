...
def train():
    ...
    if l_in is None:
        raise ValueError("Could not find a valid sample in the dataset to detect dimensions.")
        
    print(f"Detected dimensions: Ligand={l_in}, Protein={p_in}")
    model = GNNGNNModel(ligand_in_dim=l_in, protein_in_dim=p_in, hidden_dim=256).to(device)
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "research/gnn_gnn_experiment/best_model_3d_esm.pth")
            print("Model saved!")
