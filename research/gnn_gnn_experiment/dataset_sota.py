...
class SOTABindingDataset(Dataset):
    ...
    def augment_coords(self, coords):
        if self.augment:
            ...
            rotation = Rotation.from_euler(
                "xyz", np.random.uniform(0, 360, 3), degrees=True
            )
    ...
    def get_esm_embeddings(self, sequence):
        ...
        return results["representations"][self.repr_layer][
            0, 1 : len(sequence) + 1
        ].cpu()
    ...
    def build_protein_graph(self, coords):
        ...
        return torch.tensor(edge_index, dtype=torch.long), torch.tensor(
    ...
    def __getitem__(self, idx):
        ...
        if coords is None or len(sequence) < 5:
            return None
